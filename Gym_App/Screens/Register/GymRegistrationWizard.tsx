...
const GymRegistrationWizard = ({ navigation }) => {
  ...
  const handleNestedChange = (parent: keyof GymData, field: string, value: string) => {
    setGymData((prev) => ({
      ...prev,
      [parent]: typeof prev[parent] === 'object' ? { ...prev[parent], [field]: value } : prev[parent],
    }));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post("https://gym-dhlm.onrender.com/Register/Gyms", gymData);
    ...
    <ScrollView contentContainerStyle={{ padding: 20 }}>
      {step === 1 && (
        <View>
          ...
          <TextInput 
            ...
            maxLength={10} 
            onChangeText={(text) => handleChange("contactNumber", text.replace(/\D/g, ""))} 
          />
          <TextInput placeholder="Email" keyboardType="email-address" onChangeText={(text) => handleChange("email", text)} />
      ...
      {step === 2 && (
        <View>
          ...
          <TextInput placeholder="Address" onChangeText={(text) => handleNestedChange("location", "address", text)} />
          <TextInput placeholder="City" onChangeText={(text) => handleNestedChange("location", "city", text)} />
          <TextInput placeholder="State" onChangeText={(text) => handleNestedChange("location", "state", text)} />
          <TextInput placeholder="Zip Code" keyboardType="numeric" onChangeText={(text) => handleNestedChange("location", "zipCode", text)} />
      ...
      {step === 3 && (
        <View>
          ...
          <TextInput placeholder="Gym Type" onChangeText={(text) => handleNestedChange("facilities", "gymType", text)} />
          <TextInput placeholder="Weekday Hours"onChangeText={(text) => handleNestedChange("facilities", "operatingHours.weekdays", text)} />
          <TextInput placeholder="Weekend Hours" onChangeText={(text) => handleNestedChange("facilities","operatingHours.weekends", text)} />

...
import {
  ...
  StatusBar, ActivityIndicator, Image, Pressable
} from 'react-native';
...
type SearchResultsProps = NativeStackScreenProps<RootStackParamList, 'SearchResults'>;
...
const SearchResults: React.FC<SearchResultsProps> = ({ navigation, route }) => {
  ...
  const processPlaceToGym = (place: GooglePlace, distance?: string): GymResult => ({
  ...
  const searchGyms = async (placeDetails: GooglePlaceDetail): Promise<void> => {
    ...
    try {
      ...
      if (placeDetails) {
        ...
        results.push(processPlaceToGym(placeDetails as unknown as GooglePlace, distance));
      }
      ...
      if (selectedFilter === 'nearby' && userLocation) {
        filteredResults = results.sort((a, b) => {
          const aDistance = a.distance ? parseFloat(a.distance.split(' ')[0]) : 999;
          const bDistance = b.distance ? parseFloat(b.distance.split(' ')[0]) : 999;
    ...
    return (
      <SafeAreaView className="flex-1 bg-white">
        ...
        <View className="flex-row items-center justify-between p-3 border-b border-gray-200">
          <Pressable
            className="p-2"
            onPress={handleBackFromSpline}
          >
            <Ionicons name="arrow-back" size={24} color="#374151" />
          </Pressable>
          <Text className="text-lg font-bold text-gray-800">{selectedGym.gymName}</Text>
          <Pressable
            className="p-2"
        ...
        <TouchableOpacity
          ...
          onPress={handleContinueToGym}
        >
          <Text className="text-white text-center font-bold">View Gym Details</Text>
        </TouchableOpacity>
      </SafeAreaView>
    );
  }
          ...
          <GooglePlacesAutocomplete
            ...
            renderRow={(data) => (
              <View className="flex-row items-center">
                <Ionicons name="location-outline" size={16} color="#6b7280" style={{ marginRight: 12 }} />
                <Text className="text-sm text-gray-700">{data.description || data.structured_formatting?.main_text}</Text>
              </View>
            )}
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <View className="relative h-36">
                ...
                {item.source === 'google' && (
                  <View className="absolute top-2 right-2 px-2 py-1 rounded-md bg-black bg-opacity-60" 
                    style={{ position: 'absolute', top: 8, right: 8, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, backgroundColor: 'rgba(0,0,0,0.6)' }}>
                    <Text className="text-xs font-bold text-white" style={{ fontSize: 10, fontWeight: 'bold', color: 'white' }}>
                      Google
                    </Text>
                  </View>
                )}
                {item.distance && (
                  <View className="absolute bottom-2 right-2 px-2 py-1 rounded-md bg-blue-600"
                    style={{ position: 'absolute', bottom: 8, right: 8, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, backgroundColor: '#2563eb' }}>
                    <Text className="text-xs font-bold text-white" style={{ fontSize: 10, fontWeight: 'bold', color: 'white' }}>
                      {item.distance}
                    </Text>
                  </View>
              ...
              <View className="p-3">
                <Text className="text-base font-bold text-gray-800 mb-1" style={{ fontSize: 16, fontWeight: 'bold', color: '#1f2937', marginBottom: 4 }}>
                  {item.gymName}
                </Text>
                <Text className="text-sm text-gray-500 mb-2" style={{ fontSize: 14, color: '#6b7280', marginBottom: 8 }}>
                  {item.location.address}
                </Text>
                <View className="flex-row items-center">
                  <Text className="text-amber-500 mr-1" style={{ color: '#f59e0b', marginRight: 4 }}>
                    {'★'.repeat(Math.round(item.rating || 0))}
                  </Text>
                  <Text className="text-sm text-gray-800" style={{ fontSize: 14, color: '#1f2937' }}>
        ...
        <View className="flex-1 justify-center items-center px-8">
          ...
          <Text className="mt-4 text-base text-gray-500 text-center">
            Search for gyms by name, location, or tap "Nearby" to find gyms near you
          </Text>
          {userLocation && (
            <TouchableOpacity
